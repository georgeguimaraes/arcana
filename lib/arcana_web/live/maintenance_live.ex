defmodule ArcanaWeb.MaintenanceLive do
  ...
  def handle_event("rebuild_graph", _params, socket) do
    ...
    socket = assign(socket, rebuild_graph_running: true, rebuild_graph_progress: %{current: 0, total: 0})
    ...
    socket =
      case result do
        {:ok, %{entities: entities, relationships: relationships}} ->
          ...
          |> load_data()
          |> put_flash(:info, "Rebuilt graph: #{entities} entities, #{relationships} relationships")
