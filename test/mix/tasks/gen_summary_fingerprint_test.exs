defmodule Mix.Tasks.Arcana.Graph.Gen.SummaryFingerprintTest do
  use ExUnit.Case, async: true

  import ExUnit.CaptureIO

  alias Mix.Tasks.Arcana.Graph.Gen.SummaryFingerprint
  ...
  @otp_app :arcana_summary_fingerprint_test

  defmodule DefaultPrivRepo do
    use Ecto.Repo, otp_app: :arcana_summary_fingerprint_test, adapter: Ecto.Adapters.Postgres
  end
  ...
  setup_all do
    Application.put_env(@otp_app, CustomPrivRepo, priv: "priv/summary_fingerprint_custom_priv")
  ...
  defp generated_migration(dir) do
    Path.wildcard(Path.join(dir, "*_add_arcana_community_summary_fingerprint.exs"))
  end
  ...
  test "writes the migration where ecto.migrate reads it" do
    ...
    capture_io(fn -> SummaryFingerprint.run(["--repo", repo]) end)

    assert [path] = generated_migration(dir)
    assert File.read!(path) =~ "defmodule #{repo}.Migrations.AddArcanaCommunitySummaryFingerprint"
    assert File.read!(path) =~ "add :summary_fingerprint, :string"
  end
  ...
  test "honors a repo with a configured :priv" do
    ...
    assert dir == "priv/summary_fingerprint_custom_priv/migrations"
