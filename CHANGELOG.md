...
### Enhancements
  ...
  * [Arcana.VectorStore.Pgvector] Support `:hnsw_ef_search`, per call or as a global search default. An HNSW index scan takes `hnsw.ef_search` candidates before applying the query's filters, so a collection- or source-scoped search could return fewer rows than exist; raising it trades work per query for recall. It was named in the docs as a backend knob without ever being implemented. Applies to `:vector` and `:hybrid`; `:keyword` never touches the vector index
