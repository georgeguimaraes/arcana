...
### Backwards incompatible changes
  ...
  * [Arcana.Migration] `down/1` refuses to roll back when the version table exists but carries no recognised marker, instead of quietly dropping nothing. That state means either a host comment replaced the marker or the install predates versioned migrations, and in both the tables really are there. The error names the `COMMENT ON TABLE` to run to record the applied version and retry. A database with no tables at all is still a silent no-op, since that is the one case where nothing needs dropping
